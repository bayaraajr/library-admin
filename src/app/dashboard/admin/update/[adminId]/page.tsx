...
const RegisterPage: FC<any> = ({ params }: { params: { adminId: string } }) => {
    ...
    const onSubmit = async (values: IAdmin) => {
        try {
            await axios.put("/api/admin/" + adminId, values);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("/api/admin/find/" + adminId);
        ...
        <Grid container spacing={2} item xs={12}>
            <Grid item xs={12}>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Typography> Админ шинэчлэх </Typography>
